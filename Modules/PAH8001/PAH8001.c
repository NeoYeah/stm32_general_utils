...
{
    return HAL_I2C_WriteByte(&PAHi2c, PAHAddr, reg, data);
}

uint8_t PAH8001_readRegister(uint8_t reg)
{
    return HAL_I2C_ReadByte(&PAHi2c, PAHAddr, reg);
}
...
void PAH8001_init(void)
{
    ...
    PAH8001_writeRegister(0x0A, 0xF1);			//@yehangyang Sleep1_Setting set frequency time
...
int8_t PAH8001_HRD(uint8_t *result)
{
        ...
        //	}
    {
        ...
        HAL_I2C_ReadBytes(&PAHi2c, PAHAddr, 0x65, result, 3);

        //		gPAH8001Data[0] = PAH8001_readRegister(0x64);
        //		gPAH8001Data[1] = PAH8001_readRegister(0x65);
        //		gPAH8001Data[2] = PAH8001_readRegister(0x66);
        //		gPAH8001Data[3] = PAH8001_readRegister(0x67);

        //Only support burst read (0x1A~0x1C), when using I2C interface
        //		HAL_I2C_ReadBytes(&PAHi2c, PAHAddr, 0x1A,&gPAH8001Data[4],3);
        //		gPAH8001Data[4] = PAH8001_readRegister(0x1A);
        //		gPAH8001Data[5] = PAH8001_readRegister(0x1B);
        //		gPAH8001Data[6] = PAH8001_readRegister(0x1C);

        PAH8001_writeRegister(0x7F, 0x00); //bank0

        //bit7 is Touch Flag (bit7=1 is meant Touch, and bit7=0 is meant No Touch)
        //		gPAH8001Data[7]=(PAH8001_readRegister(0x59)&0x80); //Check Touch Flag
        return 1;
    }
}
...
bool PAH_readData(uint8_t *result)
{
    ...
    {
        //Only support burst read (0x64~0x67), when using I2C interface
        HAL_I2C_ReadBytes(&PAHi2c, PAHAddr, 0x64, result, 4);
        PAH8001_writeRegister(0x7F, 0x00); //bank0
...
bool PAH_isTouched(void)
{
    ...
    //Check Touch Status for power saving
    temp = PAH8001_readRegister(0x59) & 0x80;	//Touch_Detect
    ...
    {
        //No touch status to enter sleep mode
        PAH8001_writeRegister(0x05, 0xB8);
        PAH8001_writeRegister(0x7f, 0x01); 		//bank1
        PAH8001_writeRegister(0x42, 0xA0);
