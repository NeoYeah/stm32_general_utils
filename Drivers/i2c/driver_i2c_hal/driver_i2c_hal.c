...
uint8_t HAL_I2C_WriteByte(I2C_HandleTypeDef *hi2c, uint8_t addr, uint8_t reg, uint8_t data)
{
    ...
    if(HAL_I2C_Master_Transmit(hi2c, addr, Data, 2, 1000) != HAL_OK)//
    {
...
uint8_t SD_I2C_WriteBytes(I2C_HandleTypeDef* I2Cx, uint8_t device_address, uint8_t register_address
...
{
    uint8_t REG_data;
    if(HAL_I2C_Master_Transmit(hi2c, addr, &reg, 1, 1000) != HAL_OK)//@yehangyang 4/1/2017		将while改成了if[不能改成if]
    {
...
uint8_t HAL_I2C_ReadBytes(I2C_HandleTypeDef *hi2c, uint8_t addr, uint8_t reg, uint8_t *pData, uint16_t size)
{
    ...
    if(HAL_I2C_Master_Transmit(hi2c, addr, &reg, 1, 1000) != HAL_OK)
    {
        return HAL_I2C_GetError(hi2c);
    }

    if(HAL_I2C_Master_Receive(hi2c, addr + 1, pData, size, 1000) != HAL_OK)
    {
        return HAL_I2C_GetError(hi2c);
    }
